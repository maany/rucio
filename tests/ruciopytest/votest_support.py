...
- :func:`collect_votest_paths` reimplements ``collect_tests`` exactly, emitting
  **repo-relative** ``tests/test_X.py`` paths (atlas=36, belleii=52 files),
  silently dropping allow/deny entries that don't exist on disk (parity-critical).
...
def load_matrix(matrix_path: "Path") -> dict:
    ...
    Args:
        matrix_path: Path to the matrix YAML file.

    Returns:
        The parsed mapping of policy name -> policy config.
    """
...
def collect_votest_paths(matrix: dict, policy: str, repo_root: "Path") -> "list[str]":
    ...
    Args:
        matrix: The parsed matrix mapping (see :func:`load_matrix`).
        policy: The policy name (e.g. ``"atlas"``, ``"belleii"``).
        repo_root: The rucio repository root.

    Returns:
        A sorted list of repo-relative test paths (deterministic baseline).
    """
...
def rewrite_policy_section(rucio_cfg: str, config_overrides: dict) -> None:
    ...
    Args:
        rucio_cfg: Path to the live ``rucio.cfg``.
        config_overrides: Mapping of policy keys to set under ``[policy]``.
    """
...
def resolve_policy(config, env) -> "Optional[str]":
    ...
    Args:
        config: A pytest config exposing ``getoption("policy", default=None)``.
        env: A mapping (typically ``os.environ``) consulted for ``POLICY``.

    Returns:
        The resolved policy name, or ``None`` if neither is set.
    """
