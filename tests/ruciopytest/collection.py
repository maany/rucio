...
from . import suite_profile_key, delegate_to_container_key
...
def _resolve_infra(infra_arg: str) -> tuple[set[str], set[str]]:
    ...
    Returns:
        Tuple of (resolved_profile_names, raw_service_names).
    """
...
def _infer_suites_from_infra(profiles: set[str]) -> list[SuiteProfile]:
    ...
    Args:
        profiles: Set of resolved compose profile names.

    Returns:
        List of matching ``SuiteProfile`` instances.
    """
    matching = []
...
def _detect_overlaps(
    ...
    Returns:
        Dict mapping "suite1,suite2" overlap keys to lists of test node IDs.
    """
