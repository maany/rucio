...
class TestTPC(unittest.TestCase):
    ...
    def setUp(self):
        ...
        os.system('dd if=/dev/urandom of=%s/data.raw bs=1024 count=1024' % TestTPC.tmpdir)
        ...
        protocol = rsemanager.create_protocol(self.rse_xrd1_id, 'write')
        protocol.connect()
        ...
        self.rse_xrd2_file = {'name': '2_rse_remote_tpc.raw', 'scope': 'user.%s' % self.root,
         'adler32': adler32('%s/1_rse_local_put.raw' % self.tmpdir),
         'filesize': os.stat('%s/1_rse_local_put.raw' % self.tmpdir)[os.path.stat.ST_SIZE]},
        ...
        add_replicas(rse_id=self.rse_xrd2_id, files=self.rse_xrd2_file, account=self.root)


    def tearDown(self):
        delete_replicas(rse_id=self.rse_xrd1_id, files=self.rse_xrd1_file)
        delete_replicas(rse_id=self.rse_xrd2_id, files=self.rse_xrd2_file)
